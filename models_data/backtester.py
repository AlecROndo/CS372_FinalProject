...
back_data = {
    f.replace(".nc", ""): xr.open_dataset(os.path.join(_ROOT, "backtester_data", f))
    for f in os.listdir(os.path.join(_ROOT, "backtester_data")) if f.endswith(".nc")
}


btc_back = xr.open_dataset(os.path.join(_ROOT, "btcdata", "btc_back.nc"))
